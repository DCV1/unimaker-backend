...
def deforum_api(_: gr.Blocks, app: FastAPI):  
    ...
    # mksoo
    # show video
    @app.get("/deforum_api/show_video/")
    async def stream_video(path: str):
        # 제작되고 있는 중에는 progress 과정을 전송해주자.
        video_path = path
        file_like = await aiofiles.open(video_path, mode="rb")
        return StreamingResponse(file_like, media_type="video/mp4")

    # Show details of all jobs across all batches
